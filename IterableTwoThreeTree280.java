...
public class IterableTwoThreeTree280<K extends Comparable<? super K>, I extends Keyed280<K>> extends TwoThreeTree280<K, I> implements KeyedDict280<K, I> {
    ...
    protected Pair280<TwoThreeNode280<K, I>, K> insert(TwoThreeNode280<K, I> root,
        ...
        if (!root.isInternal()) {
            ...
            if (oldLeaf.next() == null) {
                ...
                LinkedLeafTwoThreeNode280<K, I> hold = oldLeaf.next();
                hold.setPrev(newLeaf);
            }
    ...
    protected void delete(TwoThreeNode280<K, I> root, K keyToDelete) {
        if (root.getLeftSubtree().isInternal()) {
            ...
            if (root.getLeftSubtree().getKey1().compareTo(keyToDelete) == 0) {
                ...
                LinkedLeafTwoThreeNode280<K, I> deleteLeft = (LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree();

                LinkedLeafTwoThreeNode280<K, I> prevD = deleteLeft.prev();
                LinkedLeafTwoThreeNode280<K, I> nextD = deleteLeft.next();

                //adjusting smallest when delete left leaf
                if (prevD != null) {
                    prevD.setNext(nextD);
                } else {
                    this.smallest = nextD;
                ...
                LinkedLeafTwoThreeNode280<K, I> deleteMiddle = (LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree();
                LinkedLeafTwoThreeNode280<K, I> prevD = deleteMiddle.prev();
                LinkedLeafTwoThreeNode280<K, I> nextD = deleteMiddle.next();

                //adjusting smallest or biggest when deleting middle leaf
                // depending on the existence of next and prev of the deleting node
                if (prevD != null) {
                    prevD.setNext(nextD);
                } else {
                    this.smallest = nextD;
                ...
                //adjusting biggest when delete right leaf
                LinkedLeafTwoThreeNode280<K, I> deleteRight = (LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree();
                LinkedLeafTwoThreeNode280<K, I> prevD = deleteRight.prev();
                LinkedLeafTwoThreeNode280<K, I> nextD = deleteRight.next();
    ...
    public K itemKey() throws NoCurrentItem280Exception {
        ...
        if (itemExists()) {
            ...
            throw new NoCurrentItem280Exception("Can not obtain item key if item not exist.");
        }
    }
    ...
    public I item() throws NoCurrentItem280Exception {
        ...
        if (itemExists()) {
            ...
            throw new NoCurrentItem280Exception("Can not obtain item data if item not exist.");
        }
    }
    ...
    public void search(K k) {
        ...
        LinkedLeafTwoThreeNode280<K, I> itemCheck;
        itemCheck = (LinkedLeafTwoThreeNode280<K, I>) super.find(this.rootNode, k);
        if (itemCheck != null) {
            this.prev = itemCheck.prev();
            this.cursor = itemCheck;
        } else {
            this.goAfter();
        }
    ...
    public void setItem(I x) throws NoCurrentItem280Exception,
        ...
        if (this.itemExists()) {
            if (x.key() == this.cursor.getData().key()) {
                this.cursor.setData(x);
            } else {
                throw new InvalidArgument280Exception("Can not set item data because ket not match each other");
            }
        } else {
            throw new NoCurrentItem280Exception("Can not set item if it not exist at first place.");
        }
    }
    ...
    public void deleteItem() throws NoCurrentItem280Exception {
        ...
        if (this.cursor != null) {
            this.delete(this.cursor.data.key());
            this.goForth();
        } else {
            throw new NoCurrentItem280Exception("Can not delete an item if it not exist at first place");
        }
    }
    ...
    public static void main(String args[]) {
        ...
        // test deleteItem
        T.delete("Plate Armor");
        if (T.itemExists() && T.item().key() == "Plate Armor")
            System.out.println("Plate Armor still in current item although we delete Plate Armor ");

        //test search method
        T.search("Blue Ioun Stone");
        if (T.item().key() != "Blue Ioun Stone") {
            System.out.println(" cursor need to be placed on on Blue Ioun Stone, but it not.");
        }

        // test search when an item not exist
        try {
            T.search("Blue Knight shield");
        } catch (Exception e) {
            System.out.println("Item should not be found because it not exist.");
        }


        //test setItem method
        Loot VampireSword = new Loot("Vampiric Blade", 10000);
        T.search("Vampiric Blade");
        T.setItem(VampireSword);
        try {
            if (T.obtain(VampireSword.key()).itemValue() != 10000)
                System.out.println("New gold value of Vampire blade should change to 12000 ");
        } catch (Exception e) {
            System.out.println("Error: unexpected exception.");
        }


        // test goFirst method before delete first
        T.goFirst();
        if (T.item().key() != "+1 Mace") {
            System.out.println("cursor in goFirst method have wrong behavior");
        }

        // test delete initial first and last item (different kind of leaf) before adjusting cursor test
        T.delete("+1 Mace");
        T.delete("Vampiric Blade");


        // test goBefore and go forth method
        T.goBefore();  // test before
        if (!T.before()) {
            System.out.println("Cursor should be in before position");
        }

        T.goForth();
        if (T.item().key() != "Blue Ioun Stone") {
            System.out.println("cursor in goBefore and goForth method have wrong behavior");
        }

        // test goAfter method after delete last

        T.goAfter();
        if (!T.after()) {   // test after
            System.out.println("Cursor should be in after position");
        }

        if (T.prev.getData().key != "Potion of Healing") {
            System.out.println("cursor in goAfter method have wrong behavior");
        }

        //test itemKey method
        T.goFirst();

        if (T.itemKey() != "Blue Ioun Stone") {
            System.out.println("itemKey method has wrong behavior");
        }

        //test keyItemPair method
        T.goForth();
        if (T.keyItemPair().firstItem() != "Leather Armor") {
            System.out.println("keyItemPair method has wrong behavior");
        }
        T.goForth();
        if (T.keyItemPair().firstItem() != "Potion of Healing") {
            System.out.println("keyItemPair method has wrong behavior");
        }

        // test goForth after we go through all items
        T.goForth();
        if (!T.after()) {
            System.out.println("Cursor should be in after position");
        }
